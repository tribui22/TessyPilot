$Global:Config = @{
    Module = "ucDrv_2E_Cfg"
    Folder = "BSW"
    TessyProject = "sk336_t2"
    TestCollection = "SK336_TC"
    ScriptRoot = "C:\Data\_DevTools\razorcat\TESSY_5.1\bin"
    TestObjects = "ucDrv_CfgClockSelfTst"
    WorkDir = "C:\Users\buimi1\Documents\03_repos\TessyPilot\Tessy_Ref_Skoda_Compact"
    SourceDir = "C:\Users\buimi1\Documents\03_repos\e-gsh_non-autosar_bsw_sk336-rcl-impl\source\project\bsw\36_BSW\SystemStack\"
}
